...
(unless (daemonp)
  ...
  (unless noninteractive
    ...
    ;; Start the scratch buffer without the default introductory text.
...
;; Finder-launched and daemonized Emacs instances may not inherit the
;; interactive shell environment.  Load the allowlisted snapshot generated by
;; scripts/common/env.sh for those macOS startup modes only.  Terminal Emacs
;; keeps the environment supplied by its parent shell.
(when (and (eq system-type 'darwin)
           (or (daemonp)
               (memq initial-window-system '(mac ns))))
  (let ((file (locate-user-emacs-file ".env.local"))
        envvars)
