...
#+end_src
** 模式编辑
最初入坑使用的是 Doom，不习惯 Evil 后切换至与原生按键不冲突的 [[https://github.com/meow-edit/meow][meow]]，配合 sis 在模式间切换输入法甚是爽利。尤其爽利的是配合 [[https://github.com/roife/emt][emt]]（仅限于 MacOS），利用 AI 完成了 =meow-word= 对于中文的分词选中。
** UI
*** 字体
